...
权威区块和时钟快照。`Game::save::path` 统一提供世界存档根路径，玩家数据、世界
元数据和 Region 文件必须位于同一个世界目录，确保创建、删除和加载生命周期一致。

玩家、世界元数据和应用设置等小型文档统一通过 `Engine::document` 编码为
`magic + 外层格式号 + gzip(MessagePack named map)`。当前业务结构使用
`#[serde(default)]` 处理新增字段，删除和未知字段默认忽略；外层格式号只在编码、
压缩或加密方式变化时升级。既有 bincode/JSON 只保留在按布局能力命名的冻结读取器中，
读取后写回即升级，不再按每次字段修改增加 `V1/V2/V3` 结构体。

Region 容器保持现有寻址与原子写入职责，单区块载荷使用具名 Section。已知 Section
按名称加载，未知 Section 按长度跳过；树实例在自身 Section 内使用字段 TLV，身份字段
缺失时拒绝加载，普通新增字段使用语义默认值。区块异步加载先查询待写和飞行中快照，
两者都不存在时才读取磁盘，并使用派发时实体身份拒绝卸载重载后的旧任务结果。
...
`Game::world::generation` 负责基础区块的确定性体素生成，管线顺序固定为
`地形 -> 洞穴 -> 矿石`：先由 `TerrainGenerator` 按生物群系参数填充地形，再由
`apply_caves` 用 3D 噪声把深部石头挖空，最后 `apply_ores` 只在洞穴壁的剩余石头上
投放矿脉。顺序不可调换——洞穴先挖空，矿石才会留在洞穴壁上而非洞穴内部。

洞穴参数当前为 `cave.rs` 内的 `DEFAULT_CAVE_PROFILE` 常量表（阈值/缩放/深度带/
天花板厚度），矿脉参数来自 Content 层的 `ore_veins` 数据驱动定义。洞穴在早期
开发阶段采用常量表，手感稳定后可按矿脉模式迁移为数据驱动。任何修改都会改变
同一种子的地形结果，必须同时评估并更新 `CURRENT_GENERATION_VERSION`。
...
已加载树苗在首次游戏分钟结算时创建 `TreeInstance`，随后按内容定义依次推进
`Sapling -> Young -> Mature`。类型和索引规则属于 `Game::world::vegetation::instance`，权威实例
集合由 `WorldState` 持有，并且只随树根所在区块保存一次；跨区块树冠不复制实例。体素与树实例
在卸载、自动保存和完整保存时组成同一聚合快照。
区块载荷由 `Game::save::world::chunk::codec` 使用独立 magic、外层格式号、具名 Section
和树字段 TLV 编解码；没有格式头的历史区块迁移为空实例集合，旧顺序式布局只读，
未知外层格式不得退化为重新生成地形。

运行时植被不得依赖 Client，也不得使用生成期 `pending_writes`。每次阶段推进会先校验完整目标
树形；跨入未加载区块或遇到与当前阶段不一致的非空气方块时整体延后，不能留下残缺结构。旧阶段
枝叶不会因形态变化而被主动删除。当前仅有方块 ID，无法区分旧树体素与玩家放回原位的同 ID
方块；需要绝对保护这类编辑前，必须先接入持久化体素来源标记。自然生成树当前仍只有体素，
尚未实例化为逻辑树；衰老、死亡与森林竞争规则也尚未接入。
