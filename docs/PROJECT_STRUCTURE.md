...
玩家种植的树苗成功生长时会创建 `TreeInstance`。类型和索引规则属于
`Game::world::vegetation::instance`，权威实例集合由 `WorldState` 持有，并且只随树根所在区块
保存一次；跨区块树冠不复制实例。体素与树实例在卸载、自动保存和完整保存时组成同一聚合快照。
...
方块时，本次生长整体延后，不能覆盖玩家建筑或留下残缺结构。自然生成树当前仍只有体素，
尚未实例化为逻辑树；幼树、衰老、死亡与森林竞争规则也尚未接入。
