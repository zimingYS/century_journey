...
/// 平面物品厚度渲染器。
///
/// 这里不再依赖贴图 UV 和 Alpha Mask 来裁剪手持模型。
/// 构建网格时直接读取 PNG 像素：不透明像素生成几何，像素颜色写入顶点颜色。
/// 这样能避免贴图过滤、UV 采样或材质裁剪导致“只显示几个像素”的问题。
