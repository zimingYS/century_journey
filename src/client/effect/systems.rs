...
/// 离开世界时重置受击与提示反馈，并隐藏相关覆盖层。
pub(super) fn clear_feedback_on_exit_system(
...
/// 首次生成受击闪屏与顶部提示的 UI 节点。
pub(super) fn spawn_feedback_ui_system(
...
/// 消费本地玩家受击事件，设置闪屏与镜头创伤。
pub(super) fn receive_damage_feedback_system(
...
pub(super) fn update_feedback_ui_system(
    time: Res<Time>,
    mut damage: ResMut<DamageFeedback>,
    mut notice: ResMut<NoticeFeedback>,
...
/// 按创伤强度驱动相机抖动并随时间衰减。
pub(super) fn camera_shake_system(
    time: Res<Time>,
    mut feedback: ResMut<DamageFeedback>,
...
/// 按挖掘进度在目标方块表面绘制裂纹。
pub(super) fn draw_break_cracks_system(progress: Res<BlockBreakProgress>, mut gizmos: Gizmos) {
