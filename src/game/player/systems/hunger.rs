...
use crate::game::player::events::{DamageEvent, HealEvent, DamageSource};
...
pub fn action_cost_system(
    ...
    for mut hunger in &mut query {
        if sprinting { hunger.exhaust(0.1 * dt); }
        if jumped { hunger.exhaust(0.05); }
    }
}
    ...
    *timer = 4.0;
    ...
    for (entity, health, hunger) in &query {
        if hunger.current >= 18.0 && health.current < health.max {
            heal_writer.write(HealEvent { target: entity, amount: 1.0 });
        }
    }
}
    ...
    *timer = 4.0;
    ...
    for (entity, hunger) in &query {
        if hunger.is_starving() {
            damage_writer.write(DamageEvent { target: entity, amount: 1.0, source: DamageSource::Starvation });
        }
    }
}