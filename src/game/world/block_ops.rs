...
/// 根据世界坐标获取方块 ID
pub fn get_voxel_at_world(world_pos: IVec3, world_state: &mut WorldState) -> u16 {
...
/// 在世界坐标处设置方块 ID
pub fn set_voxel_at_world(world_pos: IVec3, block_id: u16, world_state: &mut WorldState) {
    let chunk_pos = IVec3::new(
        world_pos.x.div_euclid(CHUNK_SIZE as i32),
        world_pos.y.div_euclid(CHUNK_SIZE as i32),
        world_pos.z.div_euclid(CHUNK_SIZE as i32),
    ...
    if let Some(arc) = world_state.loaded_chunks.get_mut(&chunk_pos) {
