...
impl InventoryContainer for HotbarData {
    ...
    fn get_stack(&self, index: usize) -> Option<&crate::inventory::item::stack::ItemStack> {
        self.stacks.get(index).and_then(|s| s.as_ref())
    }

    fn get_stack_mut(&mut self, index: usize) -> Option<&mut crate::inventory::item::stack::ItemStack> {
        self.stacks.get_mut(index).and_then(|s| s.as_mut())
    }

    fn set_stack(&mut self, index: usize, stack: crate::inventory::item::stack::ItemStack) {
        if index < HOTBAR_SIZE {
            if stack.is_empty() {
                self.stacks[index] = None;
            } else {
